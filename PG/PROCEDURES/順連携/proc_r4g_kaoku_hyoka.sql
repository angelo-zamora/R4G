...
CREATE OR REPLACE PROCEDURE proc_r4g_kaoku_hyoka ( in_n_renkei_data_cd IN numeric, in_n_renkei_seq IN numeric, in_n_shori_ymd IN numeric, io_c_err_code INOUT character varying, io_c_err_text INOUT character varying )
LANGUAGE plpgsql
AS $$
...
DECLARE
   ...
   SELECT *
	FROM i_r4g_kaoku_hyoka
	WHERE saishin_flg = '1'
	  AND kazei_nendo = (SELECT MAX(kazei_nendo) FROM i_r4g_kaoku_hyoka)
	  AND kaoku_hyoka_no = (SELECT MAX(kaoku_hyoka_no) FROM i_r4g_kaoku_hyoka)
	  AND result_cd < 8;



   rec_main                          i_r4g_kaoku_hyoka%ROWTYPE;
