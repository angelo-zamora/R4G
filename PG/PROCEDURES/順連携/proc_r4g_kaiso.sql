...
CREATE OR REPLACE PROCEDURE proc_r4g_kaiso ( in_n_renkei_data_cd IN numeric, in_n_renkei_seq IN numeric, in_n_shori_ymd IN numeric, io_c_err_code INOUT character varying, io_c_err_text INOUT character varying )
LANGUAGE plpgsql
AS $$
...
DECLARE
   ...
   SELECT *
	...
	WHERE saishin_flg = '1'
	AND result_cd < 8
	AND (kazei_nendo, kaoku_kihon_rireki_no) IN (
		SELECT MAX(kazei_nendo), MAX(kaoku_kihon_rireki_no)
		FROM i_r4g_kaoku
		WHERE saishin_flg = '1'
	);
