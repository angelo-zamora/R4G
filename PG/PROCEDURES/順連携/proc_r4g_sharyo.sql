...
CREATE OR REPLACE PROCEDURE proc_r4g_sharyo ( in_n_renkei_data_cd IN numeric, in_n_renkei_seq IN numeric, in_n_shori_ymd IN numeric, io_c_err_code INOUT character varying, io_c_err_text INOUT character varying )
LANGUAGE plpgsql
AS $$
...
DECLARE
   ...
   SELECT *
	...
	AND keiji_rireki_no = (
		...
		FROM i_r4g_sharyo AS sub
		WHERE sub.keiji_kanri_no = i_r4g_sharyo.keiji_kanri_no
		)
	AND result_cd < 8;

   rec_main                          i_r4g_sharyo%ROWTYPE;
