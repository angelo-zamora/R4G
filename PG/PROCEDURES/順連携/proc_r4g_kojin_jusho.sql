...
CREATE OR REPLACE PROCEDURE proc_r4g_kojin_jusho (
         ...
         ELSE
            ...
            IF rec_lock IS NULL THEN
               BEGIN
                  ...
                  VALUES (
                     rec_main.kojin_no
                     , rec_main.adr_shikuchoson_cd
                     , rec_main.adr_machiaza_cd
                     , get_trimmed_space(rec_main.adr_todofuken)
                     , get_trimmed_space(rec_main.adr_shikugunchoson)
                     , get_trimmed_space(rec_main.adr_machiaza)
                     , get_trimmed_space(rec_main.adr_banchigohyoki)
                     , NULL
                     , NULL
                     , get_trimmed_space(rec_main.adr_jusho_katagaki)
                     , get_trimmed_space(rec_main.adr_jusho_katagaki_kana)
                     , get_hyoji_yubin_no(rec_main.adr_yubin_no)-- TODO: Ensure function exists and works
                     , CONCAT(
                        rec_main.adr_todofuken
                        , ' ', rec_main.adr_shikugunchoson
                        , ' ', rec_main.adr_machiaza
                        , ' ', rec_main.adr_banchigohyoki
                     )
                     , rec_main.adr_kokumei_cd
                     , get_trimmed_space(rec_main.adr_kokumeito)
                     , get_trimmed_space(rec_main.adr_kokugai_jusho)
            ...
            ELSE
               BEGIN
                  ...
                  SET adr_shikuchoson_cd = rec_main.adr_shikuchoson_cd
                     , adr_machiaza_cd = rec_main.adr_machiaza_cd
                     , adr_todofuken = get_trimmed_space(rec_main.adr_todofuken)
                     , adr_shikugunchoson = get_trimmed_space(rec_main.adr_shikugunchoson)
                     , adr_machiaza = get_trimmed_space(rec_main.adr_machiaza)
                     , adr_banchigohyoki = get_trimmed_spacerec_main.adr_banchigohyoki)
                     , adr_jusho_katagaki = get_trimmed_space(rec_main.adr_jusho_katagaki)
                     , adr_jusho_katagaki_kana = get_trimmed_space(rec_main.adr_jusho_katagaki_kana)
                     , adr_yubin_no = get_hyoji_yubin_no(rec_main.adr_yubin_no)
                     , adr_kakutei_jusho = CONCAT(rec_main.adr_todofuken, ' ', rec_main.adr_shikugunchoson, ' ', rec_main.adr_machiaza, ' ', rec_main.adr_banchigohyoki)
                     , adr_kokumei_cd = rec_main.adr_kokumei_cd
                     , adr_kokumeito = get_trimmed_space(rec_main.adr_kokumeito)
                     , adr_kokugai_jusho = get_trimmed_space(rec_main.adr_kokugai_jusho)
