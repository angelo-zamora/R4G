...
CREATE OR REPLACE PROCEDURE proc_r4g_tokusokuteishi_kibetsu (
...
DECLARE
   ...
   cur_main CURSOR FOR
   SELECT *
   FROM i_r4g_tokusoku_teishi_kibetsu
   WHERE saishin_flg = '1'
      AND result_cd < 8;

   rec_main                      i_r4g_tokusoku_teishi_kibetsu%ROWTYPE;
    
   cur_lock CURSOR FOR
   SELECT * 
   FROM f_tokusokuteishi_kibetsu
   WHERE seq_no_tokusokuteishi = lc_seq_no_tokusokuteishi
      AND kibetsu_key = lc_kibetsu_key;
...
BEGIN
   ...
   IF ln_para01 = 1 THEN
   BEGIN
         SELECT COUNT(*) INTO ln_del_count FROM f_tokusokuteishi_kibetsu;
   ...
   OPEN cur_main;
      LOOP
         ...
         lc_kibetsu_key := get_kibetsu_key(
                              rec_main.fuka_nendo::numeric
                              , CASE WHEN rec_main.soto_nendo IS NOT NULL OR rec_main.soto_nendo <> '' THEN rec_main.soto_nendo::numeric ELSE 0 END
                              , rec_main.zeimoku_cd --in_gyomu_shosai_cd
                              --, get_r4g_code_conv(1, 3, null, rec_main.zeimoku_cd::character varying);
                              , rec_main.kibetsu_cd
                              , rec_main.tokucho_shitei_no
                              , rec_main.jido_atena_no
                              , rec_main.tsuchisho_no
                              , CASE WHEN rec_main.jigyo_nendo_no IS NOT NULL OR rec_main.jigyo_nendo_no <> '' THEN rec_main.jigyo_nendo_no::numeric ELSE 0 END
                              , CASE WHEN rec_main.shinkoku_rireki_no IS NOT NULL OR rec_main.shinkoku_rireki_no <> '' THEN rec_main.shinkoku_rireki_no::numeric ELSE 0 END
                           );
         ...
         ELSE
            BEGIN
               UPDATE f_tokusokuteishi_kibetsu
                  SET tokusoku_teishi_ymd = get_date_to_num(to_date(rec_main.tokusoku_teishi_ymd, 'yyyy-mm-dd'))
                  , tokusoku_kaijo_jiyu = CASE WHEN rec_main.tokusoku_kaijo_jiyu IS NOT NULL OR rec_main.tokusoku_kaijo_jiyu <> '' THEN rec_main.tokusoku_kaijo_jiyu::numeric ELSE 0 END                  , tokusoku_kaijo_ymd = get_date_to_num(to_date(rec_main.tokusoku_kaijo_ymd, 'yyyy-mm-dd'))
                  , tokusoku_teishi_jiyu = rec_main.tokusoku_teishi_jiyu
         ...
         BEGIN
            -- 中間テーブル更新
            UPDATE i_r4g_tokusoku_teishi_kibetsu 
